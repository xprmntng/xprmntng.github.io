...
const LETTER_KEY_MAP = new Map();

$(function() {
    for (let i = 0; i < 6; i++) {
        for (let ii = 0; ii < 5; ii++) {
            let nameStr = '#' + i.toString() + ii.toString();
            letterSpaces.push($(nameStr));
        }
    }
    ...
    $('#keyboard > div > a').each(function() {
        let element = $(this);
        let keyText = element.text();
        if (keyText.length == 1) {
...
function processKey(key) {
    if (key.length == 1) {
        if (col < 5 && currentSpaceIndex < letterSpaces.length) {
            key = key.toUpperCase();
            currentSpace.html(key);
            currentSpaceIndex++;
            currentSpace = letterSpaces[currentSpaceIndex];
            col++;
            userWord += key;
            if (col == 5) {
                checkState = true;
...
function pickNextWord() {
    let pick = Math.floor(remainingWords.length * Math.random());
    word = remainingWords[pick].toUpperCase();
    let tmp = remainingWords.slice(0, pick);
    tmp.push(...remainingWords.slice(pick + 1));
    remainingWords = tmp;
    $('.letter').html(null).attr('class', 'letter');
    col = 0;
    row = 0;
    letter = 0;
    userWord = '';
...
function checkWord() {
    ...
    let remainingAnswerLetters = [];
    let remainingUserLetters = [];
    for (let i = 0; i < 5; i++) {
        if (userWord[i] == word[i]) {
            $('#' + row + i).addClass('good');
            LETTER_KEY_MAP.get(userWord[i]).attr('class', 'good');
        } else {
            remainingAnswerLetters.push(word[i]);
            remainingUserLetters.push([userWord[i], i]);
        }
    }
